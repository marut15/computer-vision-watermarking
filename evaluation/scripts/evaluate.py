...
def evaluate_test_set():
    ...
    # Resolve config-relative paths so the script is CWD-independent.
    ...
    _results = config['output']['results']
    if not os.path.isabs(_results):
        _results = os.path.join(_REPO_ROOT, _results)
    results_dir = os.path.dirname(_results)
