...
#   bash evaluation/scripts/jobs/eval_new_decoders.sh --outroot /tmp/new_models
#   bash evaluation/scripts/jobs/eval_new_decoders.sh --batch-size 8 --skip-robustness
#   OUTROOT=/path WORKSPACE=/wherever bash evaluation/scripts/jobs/eval_new_decoders.sh
#
...
SPECS=(
  "global_stats|decoding/configs/global_stats.yaml|decoding/checkpoints/global_stats.pth|256"
  "spectral|decoding/configs/spectral.yaml|decoding/checkpoints/spectral.pth|1024"
  "multiscale_pyramid|decoding/configs/multiscale.yaml|decoding/checkpoints/multiscale_pyramid.pth|512"
  "dual_branch|decoding/configs/dual_branch.yaml|decoding/checkpoints/dual_branch.pth|512"
)
...
for spec in "${SPECS[@]}"; do
  ...
  #    evaluation/results/metrics/test_results/<experiment.name>.json. We copy that to
  ...
  run_step "evaluate" "${CLEAN_OUT}" "${SKIP_CLEAN}" "${CLEAN_LOG}" \
    "${PYTHON}" "${EVAL_SCRIPTS}/evaluate.py" --config "${CONFIG}"
  ...
  if [[ "${SKIP_CLEAN}" -ne 1 ]]; then
    SRC_JSON="${REPO}/evaluation/results/metrics/test_results/${ARCH}.json"
    if [[ -f "${SRC_JSON}" ]]; then
