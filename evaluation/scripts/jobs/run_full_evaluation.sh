...
# outputs into deterministic locations under evaluation/results/. After it
...
#
# Usage:
#   bash evaluation/scripts/jobs/run_full_evaluation.sh                # full run
#   bash evaluation/scripts/jobs/run_full_evaluation.sh --skip-signal  # skip signal analysis
#   bash evaluation/scripts/jobs/run_full_evaluation.sh --skip-vit     # skip ViT
#   bash evaluation/scripts/jobs/run_full_evaluation.sh --batch-size 8 # bump batch (faster on big GPU)
#
...
cd "${REPO}"
LOG_DIR="${REPO}/evaluation/.full_eval/logs"
MANIFEST="${REPO}/evaluation/.full_eval/manifest.json"
mkdir -p "${LOG_DIR}" "${REPO}/evaluation/results/figures"
...
# 1. signal analysis
SIGNAL_OUTPUT="${REPO}/evaluation/results/figures/gradcam_per_bit.png"
run_step "01_signal_analysis" "${SIGNAL_OUTPUT}" "${SKIP_SIGNAL}" \
  "${PYTHON}" "${EVAL_SCRIPTS}/analysis/signal_analysis.py" \
    --metadata "${META}" --images "${IMAGES}" \
    --output-dir "${REPO}/evaluation/results/figures" \
    --checkpoint "${CKPT_RESNET}"

# 2. compare_architectures
COMPARE_MD="${REPO}/evaluation/reports/architecture_comparison.md"
run_step "02_compare_architectures" "${COMPARE_MD}" "${SKIP_COMPARE}" \
  "${PYTHON}" "${EVAL_SCRIPTS}/compare_architectures.py" \
    ...
    --chart-png "${REPO}/evaluation/results/figures/architecture_comparison.png"

# 3-5. robustness per architecture (separate output dirs so figures don't collide)
ROB_RESNET_JSON="${REPO}/evaluation/results/metrics/robustness_resnet.json"
ROB_RESNET_DIR="${REPO}/evaluation/results/figures/robustness_resnet"
mkdir -p "${ROB_RESNET_DIR}"
run_step "03_robustness_resnet" "${ROB_RESNET_JSON}" "${SKIP_ROB_RESNET}" \
  "${PYTHON}" "${EVAL_SCRIPTS}/robustness_eval.py" \
...
ROB_SEP_JSON="${REPO}/evaluation/results/metrics/robustness_separate.json"
ROB_SEP_DIR="${REPO}/evaluation/results/figures/robustness_separate"
mkdir -p "${ROB_SEP_DIR}"
...
ROB_VIT_JSON="${REPO}/evaluation/results/metrics/robustness_vit.json"
ROB_VIT_DIR="${REPO}/evaluation/results/figures/robustness_vit"
mkdir -p "${ROB_VIT_DIR}"
...
for f in \
    ...
    "${REPO}/evaluation/results/figures/architecture_comparison.png" \
    "${REPO}/evaluation/results/figures/gradcam_per_bit.png"; do
  if [[ -e "${f}" ]]; then
...
echo
n_fig=$(find "${REPO}/evaluation/results/figures" -maxdepth 1 -name '*.png' | wc -l)
echo "  ${n_fig} top-level signal/compare figures in results/figures/"
n_rob=$(find "${REPO}/evaluation/results/figures" -maxdepth 2 -path '*/robustness_*' -name '*.png' | wc -l)
