...
modes:
  ...
  1. ``--baseline-pool DIR`` - if you have a folder with multiple baseline
     ...
     evaluation/scripts/analysis/delta_dataset_analysis.py.
...
Outputs:
  - evaluation/results/metrics/delta_null.json
  - evaluation/results/figures/delta_null_radial.png

Usage:
  python evaluation/scripts/analysis/null_seed_baseline.py --baseline-pool path/to/pool
...
def parse_args():
    ...
    p.add_argument("--out-json", default=str(REPO_ROOT / "evaluation" / "results" / "metrics" / "delta_null.json"))
    p.add_argument("--out-fig", default=str(REPO_ROOT / "evaluation" / "results" / "figures" / "delta_null_radial.png"))
    return p.parse_args()
