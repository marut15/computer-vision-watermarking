...
`evaluation/scripts/figures/*.py` tools, so it can run without explicit path flags.
"""
...
from decoding.common.smoke import default_data_paths, ensure_smoke_fixture  # type: ignore  # noqa: E402
...
def _resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path]:
    ...
    output = Path(args.output_dir) if args.output_dir else (REPO_ROOT / "evaluation" / "results" / "figures")
