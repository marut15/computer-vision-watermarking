...
#
# Examples:
#   bash setup/commit_results.sh -m "ViT-B/16 final results"
#   bash setup/commit_results.sh --dry-run
#   bash setup/commit_results.sh --no-figures -m "tables only"
#   bash setup/commit_results.sh --no-push    -m "stage locally"

set -euo pipefail

usage() {
  cat <<EOF
Usage: bash setup/commit_results.sh [options]
...
stage_if_any evaluation/reports/*.md
stage_if_any evaluation/results/metrics/*.json
stage_if_any evaluation/results/metrics/test_results/*.json
stage_if_any decoding/configs/*.yaml

# Source/script changes are tracked already; safe to add the directories.
git add decoding evaluation/scripts 2>/dev/null || true

if [[ ${INCLUDE_FIGURES} -eq 1 ]]; then
  figs=( evaluation/results/figures/*.png )
  if [[ ${#figs[@]} -gt 0 ]]; then
