...
# global-pattern decoders by handing off to scripts/train_new_decoders.sh.
#
...
# Usage on the VM:
#
#     bash computer-vision-watermarking/decoding/scripts/bootstrap_and_train_new.sh
#
# Common knobs:
...
if [[ ${SKIP_TRAIN} -eq 1 ]]; then
  ...
  echo "    bash ${SCRIPTS}/train_new_decoders.sh"
  exit 0
fi
...
echo "==================================================="
exec bash "${SCRIPTS}/train_new_decoders.sh" "${TRAIN_FLAGS[@]}"
