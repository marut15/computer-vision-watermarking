...
Run from repo root:
    ...
    python decoding/scripts/test_pipeline.py
...
SCRIPTS = REPO / "decoding/scripts"
sys.path.insert(0, str(REPO))
sys.path.insert(0, str(REPO / "decoding"))
...
rc, out, elapsed = run(
    [sys.executable, str(SCRIPTS / "robustness_eval.py"), "--smoke", "--model", "resnet"],
    cwd=REPO / "decoding",
    timeout=300,
)
print(f"  exit={rc}  elapsed={elapsed:.1f}s")
