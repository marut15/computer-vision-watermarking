...
HIST_SRC="${REPO}/evaluation/results/training_logs/${NAME}.history.json"
...
run_step "evaluate" "${CLEAN_OUT}" "${SKIP_EVAL}" "${CLEAN_LOG}" \
  "${PYTHON}" "${REPO}/evaluation/scripts/evaluate.py" --config "${CONFIG}"
if [[ "${SKIP_EVAL}" -ne 1 ]]; then
  SRC_JSON="${REPO}/evaluation/results/metrics/test_results/${NAME}.json"
...
run_step "ablation" "${ABL_OUT}" "${SKIP_ABLATE}" "${ABL_LOG}" \
  "${PYTHON}" "${REPO}/evaluation/scripts/analysis/ablate_dual_branch.py" \
    --config "${CONFIG}" \
    --arch "${ARCH}" \
    --checkpoint "${CKPT}" \
...
run_step "robustness" "${ROB_OUT}" "${SKIP_ROBUSTNESS}" "${ROB_LOG}" \
  "${PYTHON}" "${REPO}/evaluation/scripts/robustness_eval.py" \
